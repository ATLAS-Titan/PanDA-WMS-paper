...
for jobs~\cite{crooks2012multi} but, so far, no MPI jobs have been considered
...
This has several benefits, including increasing throughput by lowering the time
taken to queue and schedule jobs; enabling both concurrent and sequential job
execution depending on the number of cores available to the pilot and the time
for which it remains available; and exposing a unified and consistent interface
for the scheduling of jobs, independent from the middleware of the site on which
