...
represent a set of operations performed on data sets stored in one or more input
files. Tasks are decomposed into jobs, where each job represents the task's set
of operations and a partition of the task's data set. Since 2005, a certain
amount of parallelism has been progressively introduced for job
execution~\cite{crooks2012multi} but, so far, HEP applications have not required
MPI. Jobs are supposed to be relatively self-contained, capable of setting up
their own execution environment or having a minimal set of common dependences.
...
discuss them as they are  not relevant to  an understanding of how PanDA has
been ported to supercomputers. For a full list of subsystems see
...
jobs: tasks were described as a set of jobs and then submitted to the PanDA
Server.
...
takes care of  replicating them on the required sites
...
Buffer module for a job that is bound to the site of that pilot and ready to
be executed. Task Buffer checks the global queue (i.e., the PanDA database)
and, upon availability, returns a suitable job to the Job Dispatcher. The Job
Dispatcher dispatches that job to the PanDA Pilot
...
\begin{figure}
    ...
    \caption{PanDA WMS architecture. Numbers indicates the execution process
    based on JEDI. Several subsystems and components, the architectural
    details of PanDA Pilot and the communication among components
    are abstracted for clarity.}
