...
In this paper, a workflow is a set of tasks alongside a description of their
properties and relations. Each task may have an arbitrary number of properties
like number of cores, executables, or input/output files. Tasks may have
precedence relations depending on their data dependences or any other type of
...
concurrently. As such, a workflow can be composed by a set of workloads.

The terms ``task'' and ``job'' are also used inconsistently across communities
that perform scientific computing. In this paper, a task is a \ldots
\mtnote{Are we going to use task and job as defined in Jedi and Panda Server?}

Usually, workflows are represented as graphs where tasks are vertices and
their relations edges~\cite{}. Often, graphs are supposed to be acyclic but
graphs with cycles have been used to represent workflows of workflows. In this
...
abstract workflow can be interpreted as a formal description of the
requirements for the workflow execution. Once these requirements are matched
by a set of resource capabilities, an abstract workflow becomes a concrete
workflow, i.e. a formal description of the workflow execution process and its
...
% section panda_future_roadmap (end)
...
% neither is available on your system, please use the font closest in
% appearance to Times. Avoid using bit-mapped fonts if possible. True-Type 1
% or Open Type fonts are preferred. Please embed symbol fonts, as well, for
% math, etc.
