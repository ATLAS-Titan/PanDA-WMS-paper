...
and a UnitManager.   The PilotManager submits pilots to Titan's PBS \jhanote{for
consistency should we say Moab?}\mtnote{Moab = Scheduler; Torque = Batch system.
I used PBS Batch System because it makes easier to speak about both the system
and the script submitted to it.} batch system via SAGA API
...
node and \jhanote{the Agent's Executors on the} the Agent's Executors on the
