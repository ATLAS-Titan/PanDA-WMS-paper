...
\jhanote{not sure MPI script is optimal description or ambigious.} Multiple
...
dynamically determined tasks that constitute workloads." This definition put
the focus on tasks and not workloads, which is a level above in the RADICAL
conceptual layer diagram.} NGE is a runtime system designed to execute
...
NGE exposes an API for the application  \jhanote{workload management?} layer
...
\jhanote{for consistency should we say Moab?} batch system via SAGA API
...
node and \jhanote{the Agent's Executors on the} worker nodes, and  the
...
scripts.\mtnote{Rewrite this after checking it with Andre.}\jhanote{In the interest of space and clarity, I would  remove the last sentence of this para. And merge
the first sentence with the para above.}
...
environment~\cite{XXX}. The system \jhanote{which system? ORTE or OpenMPI in
general?} transparently provides support for interprocess communication,
