...
% to Titan's PBS batch system. Job is already used for what is pulled from
% PanDA server.} 
...
and a UnitManager.   The PilotManager submits pilots to Titan's PBS  batch
system via SAGA API (Fig.~\ref{fig:arch-overview}, dash arrow), as done by
PanDA Broker to submit MPI scripts. Once scheduled, the pilot's Agent is
bootstrapped on Titan's MOM node and \jhanote{the Agent's Executors on the}
the Agent's Executors on the worker nodes, and the UnitManager schedules units
to the Agent's Scheduler (Fig.~\ref{fig:arch- overview}, solid arrow). The
Agent's Scheduler schedules the units on one or more Agent's Executor for
...
% = Batch system. I used PBS Batch System because it makes easier to speak about
% both the system and the script submitted to it.}
