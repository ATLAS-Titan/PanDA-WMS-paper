...
% \mtnote{Addresses Alexei's comment. Increased Grid capacity to 300K cores. Is
% the number of sites still 100 or should I increase that too?}
...
% \mtnote{Addresses Alexei's comment. Is the following sentence correct?}
...
% too large or their walltime is too long.
...
% It is important to note that backfill availability is computed retrospectively
% and is determined by Titan's operation team. Backfill availability represents
% the theoretical maximum of node-hours that could be utilized by ATLAS (or any
% other project) that submit under backfill mode.
...
average of 30,000 unused cores (excluding GPU cores). This equals
% approximately 270M core-hours per year,
to roughly 10\% of the overall capacity of WLCG.
...
(Fig.~\ref{fig:panda_broker}:2) and creates an MPI script, wrapping enough ATLAS
jobs' payload to fit the backfill slot. PanDA Broker submits the MPI script to
the Titan's Torque batch system via RADICAL-SAGA
...
Cori at the National Energy Research Scientific Computing Center (NERSC).
