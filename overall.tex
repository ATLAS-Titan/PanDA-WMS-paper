...
\begin{enumerate}
    ...
    factored into the choice of walltime requests?\mtnote{Asked Sergey and
    Danila to confirm these numbers and walltime request policy.} Is it
    ...
    Titan when compared to other WLCG sites (paper page 6).\mtnote{Wrote an email to Sergey asking for a clarification.}
\end{enumerate}
\fi
...
performance from \(\approx\)6,300 seconds on Lustre to \(\approx\)1,500
seconds on NFS\@. While these figures are specific to OLCF and TItan, reengineering on AthenaMP could further improve startup time on every platform. 
