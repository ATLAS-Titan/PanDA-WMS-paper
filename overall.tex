...
number of brokers per DTN could be increased. In September 2016, increasing the
number of concurrent PanDA Broker from 4 to 20 markedly improved efficiency (see
Fig.~\ref{fig:backfill-utilization}) but further research is needed to
understand whether an even larger number brokers would allow PanDA to consume a
higher percentage of backfill availability.
...
\begin{figure*}%[htp]
    ...
    \caption{Backfill availability: distribution of number of work nodes (left); distribution of walltime in minutes (center); and a scatter plot of the two variables (right). 62175 measures: mean number of work nodes available 691; mean
