...
consumed a total of 73.8M core-hours, for an average of $\approx$ 7M core-hours
a month, with a minimum of 3.3M core-hours in April 2016 and a maximum 14.8M
...
2016 to February 2017 (hereafter called experiment time window).
\jhanote{Neither figure show efficiency. Both show utilization.} Efficiency is
defined as the fraction of Titan’s cores available via backfill that was
utilized by the PanDA Brokers. \jhanote{I would plot the efficiency on the Y2
axis for both.} The brokers reached 18\% average efficiency when measured across
the experiment time window, with a minimum 8.9\% efficiency on April 2016 and a
maximum 33.5\% efficiency on February 2017. The number of total backfill cores
available was 38.1M in April 2016, and 33.1M in February 2017. This shows that
the measured difference in efficiency did not depend on a comparable difference
...
Figure~\ref{fig:100event-distrib} shows the distribution of that duration for
XXXXX events simulated on Titan. When compared to the distribution of backfill
availability on Titan in Figure~\ref{fig:backfill-distrib}, 100 events appear to
be a good compromise.

\begin{figure}[htp]
    \includegraphics[clip,width=\columnwidth]{figures/tx_simulation_distribution.pdf}
\caption{Distribution of the time taken by a Geant4 detector simulation to simulate one event. 3000 Events; 30 Titan work nodes; 16 simulations per node; 100 events per node. Average time per event \~14 min. Broad distribution from \~1 to \~40 minutes.}
\label{fig:100event-distrib}
\end{figure}

\begin{figure}[htp]
    \includegraphics[clip,width=\columnwidth]{figures/tx_simulation_distribution.pdf}
\caption{\mtnote{Placeholder for the distribution of backfill walltime availability on Titan.}}
...
(3.9\%) on Titan. PanDA Broker could fit the number of events to the walltime of
the backfill availability on the base of the distribution of
Figure~\ref{fig:backfill-distrib}. That specific number of event could then be
pulled from the PanDA Event service~\cite{panda-event-service} and given as
input to one or more simulations. Once packaged into a MPI PBS job, these
simulations would better fit backfill availability, increasing the efficiency of
PanDA Brokers.
...
duration of other stages of the ATLAS MC workflow (\S\ref{ssec:panda-titan})
more uniform. Further analysis is needed to evaluate the trade offs between
increased efficiency of resource utilization and the complexity that would be
...
Figure~\ref{fig:100event-distrib}). The allocation policy based on work nodes
and not cores used, does not justify the use of 1/2 of the cores available.

\begin{figure}[htp]
    \includegraphics[clip,width=\columnwidth]{figures/tx8_tx16_comparison.pdf}
\caption{Comparison between distributions of the time taken by a Geant4 detector simulation to simulate one event when placing 2 simulations (h1) or 1 simulation (h2) per CPU. 2 simulation use 16 cores per node, 1 simulation 8, 1 per compute unit. 50 Events; 1 Titan work nodes; 16 work threads per node; 100 events per node. Average time per event \~14 min. Broad distribution from \~1 to \~40 minutes.}
