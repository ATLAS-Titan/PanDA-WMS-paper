...
ATLAS on Titan during the experiment time window. ATLAS consumed a total of
73.8M core-hours, for an average of $\approx$ 7M core-hours a month, with a
...
Figure~\ref{fig:backfill-utilization} shows backfill utilization during the
experiment time window.\jhanote{Neither figure show efficiency. Both show
...
efficiency on the Y2 axis for both.} The brokers reached 18\% average
...
Figure~\ref{fig:hpc-workload-utilization} shows that during the experiment time
window, about 2.25M detector simulation jobs were completed on Titan, for a
total of 225M events processed. This is equivalent to 0.9\% of all the 250M
...
\caption{Distribution of the time taken by a Geant4 detector simulation to simulate one event. 3000 Events; 30 Titan worker nodes; 16 simulations per node; 100 events per node. Average time per event $\approx$ 14 min. Broad distribution from $\approx$ 2 to $\approx$ 40 minutes.}
...
availability (i.e., walltime). The mean number of nodes over the experiment
window time was $\approx$ 267, and their mean walltime was 1445 minutes.
Detector simulations of 100 events, enable to use down to 1 node for 1/4 of the
mean walltime of backfill availability. As such, it offers a good compromise for
the PanDA Broker efficiency.

\begin{figure}[htp]
    \includegraphics[clip,width=\columnwidth]{figures/avail_time_vs_nodes_Oct_28_Nov_3_2015.png}
\caption{\mtnote{Waiting to know whether we have data for the experiment time window.}}
...
\begin{itemize}
    ...
    \item Summary overall figure of performance analogous to those produced with NGE. Diagram as discussed in F2F meeting at Rutgers. (waiting)
\end{itemize}
...
lower that that of LCWG. Figure~\ref{fig:comparison-titan-grid} shows the
difference in execution time of the same type of simulation between Titan and
